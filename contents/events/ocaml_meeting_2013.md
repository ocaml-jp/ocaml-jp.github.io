---
page_title: OCaml Meeting 2013 in Nagoya
title: OCaml Meeting 2013 in Nagoya
date: 2013-08-24
---

OCamlミーティングは、OCamlユーザーによる技術交流会です。

OCamlは古くからソースコード互換性を維持している一方で、近年も積極的に拡張が取り入れらていたり、開発環境やライブラリが新しくなっています。今年はそんなOCamlの最新事情をキャッチアップできる機会になればと思っています。

ちなみに午後は同じ場所で[ProofSummit?](http://ocaml.jp/?cmd=edit&page=ProofSummit&refer=um2013)が行われます。そちらも奮ってご参加下さい。

### 日時・場所

* 日程：2013/8/24(土)
* 会場：名古屋大学　多元数理科学研究科　理1号館 509号室
* 交通アクセス：[http://www.math.nagoya-u.ac.jp/ja/direction/campus.html](http://www.math.nagoya-u.ac.jp/ja/direction/campus.html)

### プログラム

* 10:00 - 開始案内
* 10:10 - Language Update(小笠原)
* 10:40 - ocamloptの全体像(@masterq_teokure氏)
* 11:20 - herokuでOCamlアプリケーションを動かすn個(n>=0)の方法 (@mzp氏)
* 12:00 (終了)

### 参加登録

人数把握のため [http://partake.in/events/83b4baa0-63d4-4154-a6cb-15beaa9e8a46](http://partake.in/events/83b4baa0-63d4-4154-a6cb-15beaa9e8a46) にご登録ください。

### 名大周辺ランチ情報

名大周辺か地下鉄で一駅の本山まで出ると色々あります。

* Google Mapに[めぼしい店](http://maps.google.co.jp/maps/ms?gl=jp&hl=ja&brcurrent=3,0x6003700811fdc32b:0xedafba947ba12fc,0&ie=UTF8&msa=33&msid=115812227863470895801.00048bcd2861befe0a0f0&abauth=4c47e17aF97PoY4-YkDByYP6kdP4X-CO9f0)をメモっておきました。

### 会場の電源・ネットワークについて

* 延長コードを出来る限り配備してコンセントを増やしておきますが、それでも相当足りない事が予想されます。譲り合って使って頂けると幸いです。
* 大学の無線LANは動画配信で重くなると思われますので、当日希望された方にのみ接続情報をお知らせします。可能な方はモバイル回線の持参をお願い致します。

その他、ご意見・連絡事項などありましたら、 `# String.concat "." ["ogasawara@itpl"; "co"; "jp"];;` までご連絡下さい。